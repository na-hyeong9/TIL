...
for i in all_:
    a = i.director.name
    print(a, i.genre, i.title, i.opening_date, i.running_time, i.screening)
```
...
for i in all_:
    a = i.director.name
    b = i.genre.title
    print(a, b, i.title, i.opening_date, i.running_time, i.screening)
```
...
```python
all_ = Movie.objects.order_by('-opening_date')
...
```python
fir_m = Movie.objects.order_by('opening_date')[0]

print(fir_m.director.name, fir_m.genre.title, fir_m.title, fir_m.opening_date, fir_m.running_time, fir_m.screening)
```
...
```python
movie_on = Movie.objects.filter(screening=True).order_by('opening_date')
...
```python
dir_ = Director.objects.get(name = '봉준호')
movie_on = Movie.objects.filter(director = dir_).order_by('opening_date')
...
```python
dir_ = Director.objects.get(name = '봉준호')
movie_on = Movie.objects.filter(director = dir_).order_by('opening_date')[1:2]
...
```python
r_119 = Movie.objects.filter(running_time__gt=119).order_by('running_time')
...
```python
r_119_u = Movie.objects.filter(running_time__gte=119).order_by('running_time')
...
```python
open_ = Movie.objects.filter(opening_date__gt='2022-01-01').order_by('opening_date')
...
gen_d = Genre.objects.get(title='드라마')
gen_ = Movie.objects.filter(director=dir_b,genre=gen_d).order_by('opening_date')
...
```python
dir_b = Director.objects.get(name='봉준호')
exc_bong = Movie.objects.exclude(director = dir_b).order_by('opening_date')
